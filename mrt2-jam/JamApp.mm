...
- (void)connectSourceNodeForCurrentDevice {
    ...
    if (wantMC) {
        ...
        // KNOWN ISSUE: per-channel separation still doesn't reach a real 4-out
        // USB interface (e.g. EP-136) — main/click both fold onto outs 1/2. The
        // render block provably writes distinct signal to all 4 buffers, so the
        // collapse is downstream in AVAudioEngine's device routing. Setting an
        // identity AUAudioUnit.channelMap on the source node was tried and did
        // NOT fix it; the likely real fix is the AUHAL output-unit channel map
        // and/or opening the device at its native rate. See
        // docs/KNOWN_ISSUES.md "Multichannel per-channel (click) routing".
    } else {
